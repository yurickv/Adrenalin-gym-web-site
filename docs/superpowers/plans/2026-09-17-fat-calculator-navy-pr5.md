...
- Суху масу тепер отримуємо від уже округленої жирової маси
  (`round1(round1(weightKg) - fatMassKg(weightKg, pct))`), тому жирова і суха маса завжди у
  сумі дають вагу, округлену до 0,1 кг (як і обидва доданки).
- Складку для жінок скрізь названо правильно — «над клубовою кісткою» (suprailiac): у довіднику
  (коментар до формули Джексона-Поллока), підписі поля форми, підказці каліпера і в FAQ.
- Уточнено формулювання складок на грудях (посередині між передньою пахвовою лінією і соском,
  чоловіки) і на стегні (передня поверхня).
- Заголовки «Як виміряти обхвати» і «Як виміряти шкірні складки» стали `<h3>`.
- Підказку про неправдоподібний результат тепер побудовано з `PLAUSIBLE_RANGE` замість
  захардкоджених чисел і доповнено найчастішою причиною залежно від методу: обхвати
  талії і шиї для стрічки, товщина складок або вік для каліпера (окремий коміт 0d8c3c0).
- Підпис поля стегна в методі складок уточнено («передня поверхня»), щоб не плутати зі стегнами
  (обхватами) методу ВМС.
- Додано чотири нові тести: сума жирової і сухої маси при будь-якій вазі й відсотку, межі
  категорій на кожному порозі для обох статей, відповідність підписів таблиці класифікатору, і
  неправдоподібний результат при талії, що ледь перевищує шию — разом 70 тестів.
