...
- Примітка в таблиці «Норма ІМТ за віком» переписана без непідтвердженого твердження про захист
  від втрати мʼязів після 65 років: тепер пояснює, що верхня межа норми зростає з віком, бо
  невеликий запас ваги повʼязують із нижчим ризиком ускладнень.
- Підсумок у `ImtResult` більше не викликає `.toLowerCase()` для категорії — «Ожиріння I ступеня»
  й інші лишаються з великої літери, а римські цифри I/II/III не ламаються нижнім регістром.
- У `hasPart` хабу (`CalcsHubJsonLd`) кожен вузол `WebApplication` отримав
  `offers: { '@type': 'Offer', price: '0', priceCurrency: 'UAH' }`.
- Поле віку в калькуляторі ІМТ тепер без повзунка: `InputSkeleton` отримав необовʼязковий проп
  `noRange`, і лише вік у `ImtCalcList` передає його.
- Локальний `toNumber` з `ImtResult` і `CaloriesResult` замінено на спільний
  `lib/toPositiveNumber.ts` (з власним тестом `lib/toPositiveNumber.test.ts`); обидва компоненти
  імпортують `toPositiveNumber`.
- Хаб `/calcs`: картки рендеряться як `<CalcHubCard key={card.href} {...card} />` замість шести
  явних пропів; `PAGE_URL` збирається з `CALC_SITE_URL` (`@/const/calcSeo`) замість хардкодженого
  домену.
- Футер: «звязку» виправлено на «звʼязку».
- `IMTDescription`: прибрано зайвий `basis-1/2` з кореневого `div`, `25.0-29,9` виправлено на
  `25,0–29,9`, усі апострофи в тексті (прямі `'` і типографські `’`) уніфіковано до `ʼ` (U+02BC), а
  пункт про ІМТ понад 30 тепер каже «30 і більше — ожиріння I–III ступеня (див. таблицю категорій
  нижче)», зберігши речення про ризики для здоровʼя.
- H1 і підзаголовок калькулятора ІМТ винесено у спільні константи `IMT_H1`/`IMT_LEAD`
  (`components/calcs-page/imtHero.ts`), якими тепер користуються і `page.tsx`, і `loading.tsx`;
  вступний абзац `page.tsx` виправлено на «Девіна й орієнтовну» замість «Девіна і орієнтовну».
- Чотири нові тести в `lib/bmi.test.ts` (крайні значення форми, границя 152 см для Девіна, перший і
  останній рядки таблиці зросту, значення `BMI_LIMITS`) — разом із новим тестом `toPositiveNumber`
  це дає 6 файлів і 55 тестів.
- Відхилення від спеки: блок «Фітнес-калькулятори» (`CalcTitle`) на сторінці ІМТ, як і раніше,
  стоїть перед контентними таблицями (`BmiCategoryTable`, `BmiHeightWeightTable`, `BmiAgeTable`),
  дзеркалячи розташування на сторінці калорій, а не після таблиці віку, як передбачала спека.
